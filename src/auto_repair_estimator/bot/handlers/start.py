...
# Dedicated nudge for the "photo arrived while user is in manual flow"
# branch (bug #4). Separated from the text-nudge family because it must
# carry different wording — we're explaining why the photo is being
# ignored, not why the text message is.
PHOTO_DURING_MANUAL_NUDGE = (
    "Вы сейчас в режиме ручного ввода. Чтобы проанализировать фотографию, "
    "нажмите «Начать» и выберите режим «С фотографией (ML)». Текущая "
    "заявка сохранена — фото пока не обрабатывается."
)


async def handle_start(message: Message) -> None:
    await message.answer(message=WELCOME_TEXT, keyboard=mode_selection_keyboard())
